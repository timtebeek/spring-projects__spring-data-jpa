...
import static org.springframework.data.jpa.repository.query.SpringDataJpql31Utils.*;
 ...
 * @since 3.1
 */
class SpringDataJpqlListenerTests {
	...
	void fluentApi() {
		...
		// when
		var results = SpringDataJpql31Utils //
				// .withSpringJpqlListener(new SpringDataJpqlListener()) //
				.withSpringJpqlVisitor(new SpringDataJpqlVisitor()) //
				.withQuery(original) //
				.withSort(Sort.by("first_name", "last_name")) //
				.query();
	...
	void fluentApiWithDescSort() {
		...
		var results = SpringDataJpql31Utils //
				.withSpringJpqlVisitor(new SpringDataJpqlVisitor()) //
				.withQuery(original) //
				.withSort(Sort.by("first_name", "last_name").descending()) //
				.query();

		// then
		assertThat(results)
				.isEqualTo("SELECT e FROM Employee e where e.name = :name order by e.first_name desc, e.last_name desc");
	}

	@Test
	void fluentApiForCounting() {
	...
	@Test
	void fluentApiDescendingForCounting() {
	...
	@Test
	void settingDebugShouldYieldMoreInfo() {

		// given
		var original = "SELECT e FROM Employee e where e.name = :name";

		// when
		var results = queryDebug(original);

		// then
		assertThat(results) //
				.contains("SELECT[Select_clause]e[Identification_variable]") //
				.contains("FROM[From_clause]Employee[Identification_variable]e[Identification_variable]") //
				.contains(
						"where[Where_clause]e[Identification_variable].[State_field_path_expression]name[Identification_variable]=[Comparison_operator]:[Input_parameter]name[Identification_variable]");
	}
	...
	void applyingSortShouldIntroduceOrderByCriteriaWhereNoneExists() {
		...
		// when
		var results = query(original, sort);
	...
	void applyCountToSimpleQuery() {
		...
		var resultsDebug = countQueryDebug(original);

		System.out.println("\"" + resultsDebug + "\"");
	...
	void demoFailures() {
		...
		assertThatExceptionOfType(JpqlSyntaxError.class).isThrownBy(() -> {
	...
	void applySortingAccountsForNewlinesInSubselect() {
		...
		assertThat(query("select u\n" + //
				"from user u\n" + //
				"where exists (select u2\n" + //
				"from user u2\n" + //
				")\n" + //
				"", sort)).isEqualToIgnoringWhitespace("select u\n" + //
						"from user u\n" + //
						"where exists (select u2\n" + //
						"from user u2\n" + //
						")\n" + //
	...
	void doesNotPrefixOrderReferenceIfOuterJoinAliasDetected() {
		...
		assertThat(query(query, sort)).endsWith("order by p.address.city asc");
	...
	void appliesSortCorrectlyForFieldAliases() {
		...
		String fullQuery = query(query, sort);
